    ...
    final diffResult = calculateListDiff<Object>(
      ...
      equalityChecker: (item1, item2) {
        if (item1 is Map<String, Object> && item2 is Map<String, Object>) {
          final message1 = item1['message']! as types.MessageData;
          final message2 = item2['message']! as types.MessageData;

          return message1.receiverId == message2.receiverId;
        } else {
          return item1 == item2;
        }
      },
  ...
  void _scrollToBottomIfNeeded(List<Object> oldList) {
    try {
      ...
      if (oldItem is Map<String, Object> && item is Map<String, Object>) {
        final oldMessage = oldItem['message']! as types.MessageData;
        final message = item['message']! as types.MessageData;

        // Compare items to fire only on newly added messages
        if (oldMessage != message
            // && message.receiverId == InheritedUser.of(context).user.receiverId
            ) {
